...
player = Player(screen,board,LIVES,SCORE)

ghost_blue = Ghost(screen,board,"blue",800,48, GHOST_SPEED)
ghost_red = Ghost(screen,board,"red",45,48, GHOST_SPEED)
ghost_pink = Ghost(screen,board,"pink",52,48, GHOST_SPEED)
ghost_orange = Ghost(screen,board,"orange",440,440, GHOST_SPEED)
...
while run:
    ...
    if game_is_on:
        ...
        if pac_man_died :
            ...
            player = Player (screen, board, player_lives, player_score)

            if player_lvl_up:
                board = Board(screen, WIDTH, HEIGHT, player.board.board)
                ghost_blue = Ghost (screen, board, "blue", 800, 48, GHOST_SPEED)
                ghost_red = Ghost (screen, board, "red", 52, 48, GHOST_SPEED)
                ghost_pink = Ghost (screen, board, "pink", 52, 48, GHOST_SPEED)
                ghost_orange = Ghost (screen, board, "orange", 500, 390, GHOST_SPEED)
            else:
                board = Board (screen, WIDTH, HEIGHT, player.board.board)
                ghost_blue = Ghost(screen, board, "blue",  800, 48, GHOST_SPEED)
                ghost_red = Ghost(screen, board, "red", 52, 48,GHOST_SPEED)
